...
For example, this definition requires callers to supply both \texttt{a} and \texttt{b}
with named values:
...
While this one requires callers to supply \texttt{a} positionally and \texttt{b} by name,
but allows the caller to omit a value for \texttt{b}:

\begin{pygmented}[lang=lisp]
(define* (add-values a #:key b)
  (+ a b))
\end{pygmented}

This one also requires callers to supply \texttt{a} positionally and \texttt{b} by name,
but allows the caller to omit both values:
