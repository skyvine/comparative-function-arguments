...
Furthermore, lisp languages ensure that all program text can be represented as valid code
objects.
For example, when discussing the example \texttt{add-values} function throughout this
paper, I might refer to the variables by their textual representations "a" and "b".
In many languages these textual representations are not associated with distinct code
objects that can be manipulated by the programming language itself.
Macro systems and code generation tools typically try to address this deficiency by
directly manipulating the textual representation before it is given to the compiler and/or
interpreter.
\textbf{TODO: This doesn't seem entirely fair. Rust macros, Python strings, etc. Do I even
need to talk about this or am I just stanning?}
Lisp languages instead provide a direct representation of variable names, which are called
symbols.
A symbol can be represented in lisp by prefixing a variable name with a single quotation
mark:

\begin{pygmented}[lang=lisp]
(class-of 'example)
; result: #<<class> <symbol> 7f8540a21000>
\end{pygmented}

Additionally, if a paranthesized list is prefixed with a quotation mark the contents are
treated as a literal list instead of a fuction call:
...
'(add-values 1000 1001)
; result: '(add-values 1000 1001)
\end{pygmented}

All lisp dialects share these common features
\footnote{
  This is not an exhaustive list of lisp-language features, nor am I aware of a definitive
  list of features "required to be a lisp".
  An attempt to create one would be... ill-advised. =)
}.

A dialect of lisp determines which primitives exist in the language.
For example, the scheme dialect states that \texttt{define} can be used to create a
function like this:
...
Guile is an implementation of scheme, so it provides the requirements given by the
standard.
It also extends scheme with unique fuctionality such as additional functions designed for
operating on Linux systems.
...
This paper will generally focus on compiled bytecode as this keeps the analysis consistent
with other sections and is the typical way to execute Guile code\footnote{
...
When instructions take an index, they differ in whether they take in and index relative to
...
For example, this definition allows callers to omit the values of either \texttt{a} or
\texttt{b} (in which case the value \texttt{\#false} will be used), but still requires
them to be supplied positionally (if they are supplied at all):
...
This one also allows their values to be omitted, but requires them to be supplied by name:
...
This one also allows both values to be omitted, but requires \texttt{a} to be supplied
positionally and \texttt{b} to be supplied by name:
...
Additionally, authors can define default values for arguments by providing 2-element lists
instead of symbols:
...
This structure contains stores all the information about the different kinds of arguments
...
It loads the built-in function \texttt{add} (which corresponds to the \texttt{+} function)
and calls it.
It does not need to move the arguments onto the stack because they are already in the
correct locations for calling a built-in function.
...
This is done by adding \texttt{\#:allow-other-keys} to the function signature.
