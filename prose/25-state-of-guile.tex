...
While the \texttt{call-scm<-scm-scm}, which is pronounced "Call scheme from scheme scheme"
to signify that it is calling a built-in function that returns as scheme value and accepts
two scheme values as input, includes the name of the built-in function it is calling:

\begin{pygmented}[lang=lisp]
(call-scm<-scm-scm 8 8 7 111)   ;; lookup-bound
\end{pygmented}

Note that the \texttt{alloc-frame} instruction will contain a comment referring to the
...
produced the plain \texttt{call} instruction; irrelevant parts of the code (such as a
...
Instruction 0 loads the literal string \texttt{"add-values"} into index 7
...
Instructions 3-4 place the constant values 1000 and 1001 into indices 4 and 3, respectively.
