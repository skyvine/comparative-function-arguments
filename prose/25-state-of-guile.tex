...
\begin{itemize}
  ...
  \item{Normalize order of presenting call/definition. Maybe add subheadings?}
\end{itemize}
}
...
The bytecode generated for the call to \texttt{add-values} performs the function call by
loading the function, loading the arguments, and dispatching to the function.
...
Instruction 5 can be safely ignored; it handles events like signals from the operating
system.
...
the stack and its arguments can come from any stack locations
\parencite[][libguile/vm-engine.c line 1556]{guile-source}.
...
The frame's metadata is typically set by the \texttt{call} instruction
\parencite[][libguile/vm-engine.c lines 428-461]{guile-source},
while the arguments are added to the stack by the by separate instructions such as
...
When a function using named values is called, the compiler adds additional instructions
before the call.
Instruction 0, \texttt{bind-kwargs} (explained in more detail after the bytecode listing),
performs the bulk of the work required to process named values at runtime.
...
\begin{pygmented}[lang=lisp]
 1 (call-scm<-thread 7 62)         ;; current-module
 2 (static-ref 6 16329)            ;; sum
 3 (call-scm<-scm-scm 7 7 6 41)    ;; define!
 4 (make-non-immediate 6 16327)    ;; #<procedure sum (#:key a b)>
 5 (scm-set!/immediate 7 1 6)      
 6 (static-ref 3 16338)            ;; #:a
 7 (make-immediate 2 34)           ;; 8
 8 (static-ref 1 16347)            ;; #:b
 9 (make-immediate 0 38)           ;; 9
10 (make-immediate 4 4)            ;; #f
11 (handle-interrupts)
12 (call-label 3 5 8)              ;; sum at #x7f308dda2158
\end{pygmented}
...
Instructinos 2-7 fill in the default value but in this case the value is
