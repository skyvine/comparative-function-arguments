...
to signify that it is calling a built-in function that returns a scheme value and accepts
...
"r\textsuperscript{N}rs" where the N is replaced with a number representing the revision
count.
For example, the most recently published version of the standard is the 7th revision of
the Scheme programming language so it is referred to as "r\textsuperscript{7}rs".
...
arguments.
r\textsuperscript{2}rs extended this by allowing the caller to pass any number of values
...
\subsubsection{Historical Record}
The dotted list syntax was first described in r\textsuperscript{2}rs, with the same
